 ...
 */
@Service
public class GuaranteeContractService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(GuaranteeContractService.class);
    ...
    public ResponseEntity<Message> getOriginContract(String userName, String applyNum){
        ...
        String contract = guaranteeContract.getContactPdf();
        String confirmation = guaranteeContract.getRepaymentPlanPdf();
    ...
    public ResponseEntity<Message> sendMessage(String applyNum){
        GuaranteeContract guaranteeContract = guaranteeContractRepository.findByApplyNum(applyNum);
        String accountId = guaranteeContract.getAccountId();
        if(accountId == null || accountId.isEmpty()){
            accountId = SignHelper.addPersonAccount(guaranteeContract.getIdCard(), guaranteeContract.getName());
            if(accountId == null || accountId.isEmpty()){
                return new ResponseEntity<Message>(new Message(MessageType.MSG_TYPE_ERROR, "短信发送失败，创建个人账户失败"), HttpStatus.OK);
            }
        }
        boolean flag = SignHelper.sendMessage(accountId, guaranteeContract.getPhoneNum());
        if(!flag){
            logger.error("发送短信失败，手机号", guaranteeContract.getPhoneNum());
            return new ResponseEntity<Message>(new Message(MessageType.MSG_TYPE_ERROR, "短信发送失败!"), HttpStatus.OK);
        }
        guaranteeContract.setAccountId(accountId);
    ...
    public ResponseEntity<Message> sign(String applyNum, String code){
        ...
        GuaranteeContract personalInfo = guaranteeContractRepository.findByApplyNum(applyNum);
        ...
        //全部签署完成后更新担保人签约状态为完成状态
        personalInfo.setState(ContractSignStatus.SUBMIT.code());
        guaranteeContractRepository.save(personalInfo);
        //签署完成的主合同和还款计划及为主贷人的主合同和还款计划表
     ...
     * @return
     */
    public ResponseEntity<Message> signPdf(GuaranteeContract personalInfo, String type, String code){
     ...
     */
    private FileDigestSignResult doSignWithTemplateSealByStream(GuaranteeContract personalInfo, String downLoadUrl,
...
//            FileDigestSignResult platformSignResult = SignHelper.platformSignByStreammConfirm(srcPdfFile);
            PosBean posBean =setKeyPosBean("保证人签字/盖章");
