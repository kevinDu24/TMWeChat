app.controller('marketController', ['$scope', '$state', '$http', '$filter', '$localStorage', '$modal', function($scope,$state, $http, $filter, $localStorage, $modal) {

    $localStorage.requestUrl = 'app.hpl.market';
    ...
    function getStatisticData(area){
        $http.get('/sales/queryStatisticsByDate?type=1&userLevel='+area.levelId+'&startDate='+$filter('date')($scope.startDate,'yyyyMMdd')+'&endDate='+$filter('date')($scope.endDate,'yyyyMMdd')).success(function(data){
