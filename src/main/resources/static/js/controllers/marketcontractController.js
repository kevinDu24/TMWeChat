app.controller('marketcontractController', ['$scope', '$state', '$http', '$filter', '$localStorage', '$modal', function($scope,$state, $http, $filter, $localStorage, $modal) {
    ...
    function getStatisticData(area){
        $http.get('/sales/plan?parentId='+area.levelId+'&startDate='+$filter('date')($scope.startDate,'yyyyMMdd')+'&endDate='+$filter('date')($scope.endDate,'yyyyMMdd')+'&planType=7').success(function(data){
